...
Removing non-zero values for area doesn't do much to alter the view, as there are still a large number of observations in the lowest bucket of the histogram. When we alter the scale to look only at non-zero values less than 50 hectares, we see that there is no obvious cutoff point at which we can call a fire "significant." Later in our analysis, we will choose to consider any fire with burned area greater than 2 hectares to be **potentially damaging** though this cutoff values can be altered depending on the chosen sensitivity of the warning system.
