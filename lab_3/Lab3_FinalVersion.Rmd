...
```{r}
model4 = lm(crmrte ~ prbarr + prbconv + polpc + sqrt(density) + pctmin80 + pctymle + factor(west) + factor(central) + factor(urban), data=crime)
summary(model4)
par(mfrow=c(2,2))
plot(model4)
```
...
We employed Akaike test to choose which model gives us a better model as AIC test the goodness-of-fit and parsimony model. We observed that our last model 4 has the better fit albeit with only 9 explantory varibles. All the while we also maintained our $R^2$ variation at **`0.837`**. 
