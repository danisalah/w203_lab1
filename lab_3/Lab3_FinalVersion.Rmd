...
Our analysis will first explore the data to ensure appropriate quality for analysis, perform univariate and multivariate analysis on key indicators, and finallly explore proposed regression models in building levels of complexity.
...
4. any known input errors that violate the logic of the variables, such as probabilities and percentages of greater than 1
...
We first check for any empty rows in the data set and find that there aren't any.

```{r}
crime[rowSums(is.na(crime)) != ncol(crime),]
```
...
```
Lastly, we note that there are `r nrow(crime %>% filter(crmrte > 1 | prbarr > 1 | prbconv > 1 | prbpris > 1 | pctmin80 > 1 | pctymle > 1))` rows with input errors, because probabilities or percent of totals can't be greater than 1. We remove those entry errors.
...
We now add another variable, pctymle, in our regression model and observe that transforming pctymle into log(pctymle) shows a clear normal distribution. 
...
Since the observation falls within a cook's distance less than 1/2, it does not have enough influence to change the regression results. Further, our second model has an AIC score of `r AIC(model2)` which is lower than our first model's AIC score of `r AIC(model1)` so we will keep both the variable and outlier in our second model.

Our second model accounts for `r summary(model2)$r.squared` of the variation in log(crime rate) which captures slightly more variation than the first model. As it stands now, our model demonstrates that a one percent increase in the percenate of males is associated with a .48 percent increase in the crime rate, holding density and probability of arrest fixed. We also note that adding the pctymle variable didn't change the relationship between the dependent variable and other independent variables.
...
As we developed our prediction model to be more accurate and precise with parsimony model all the while not losing the goodness of fit, we further explored in our selection of variables to use as explanatory variables. As our model 3 indicated that there are many variables that has multicollinearity that would inflate our $R^2$. In addition, $R^2$ would increase as we add more variables in our model. Evidently we see in our model3. 

To further fine tune our model, we chose 9 variables as our explanatory variables. We selected 3 out of 9 variables to be our dummy variables: `west`, `central` and `urban` because we believed that crime events are different across regions and this would generate three different regression line based on the location. Those dummy variables effect are absorbed in our first model1 where we only used `density` as our explanatory variable.  

# Model 4

```{r}
model4 = lm(log(crmrte) ~ prbarr + prbconv + polpc + sqrt(density) + pctmin80 + log(pctymle) + factor(west) + factor(central) + factor(urban), data=crime)
summary(model4)
par(mfrow=c(2,2))
plot(model4)
```
...
We employed Akaike test to choose which model gives us a better model as AIC test the goodness-of-fit and parsimony model. We observed that our last model 4 has the better fit albeit with only 9 explantory varibles. All the while we also maintained our $R^2$ variation at **`0.822`**. 
